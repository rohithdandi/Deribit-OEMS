...
public:
        ...
        std::size_t bytes_transferred)
    {
        ...
        ws_.async_read(
            buffer_,
            boost::asio::bind_executor(ws_strand_, beast::bind_front_handler( &session::on_shutdown, shared_from_this())));
            // beast::bind_front_handler(
            //     &session::on_read,
            //     shared_from_this()));
    }
...
int main(int argc, char** argv){
    ...
    std::string authMessage = "Hello, world!";
    ...
    std::make_shared<session>(ioc, ctx)->run(host, port, authMessage.c_str(), endpoint);
